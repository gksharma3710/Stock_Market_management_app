import decimal
import re
from tkinter import messagebox , Frame, Listbox,ttk
from tkinter import Toplevel, Label, Entry, Button
# import wallet import update_wallet_balance
...
#     window.bind('<Escape>', lambda e: window.attributes('-fullscreen', False))  # Pressing 'Escape' will exit full-screen mode

def connect_to_database():
    """
    Connect to the Stock Market Database.

    This function establishes a connection to a MySQL database named 'sm_db' using the provided credentials.
    The connection is essential for performing database operations, such as retrieving data, updating records, and
    managing user information.

    Returns:
        mysql.connector.MySQLConnection: A connection object to the 'sm_db' database.
...
def process_payment(user_id, card_number, expiry_date, cvv, cardholder_name, billing_address):
    try:
        # Check if the payment credentials already exist for the user
        cursor.execute("SELECT user_id FROM payment_credentials WHERE user_id = %s", (user_id,))
        result = cursor.fetchone()
        if result:
            # Update existing payment credentials
            cursor.execute("""
                UPDATE payment_credentials 
                SET card_number = %s, expiry_date = %s, cvv = %s, cardholder_name = %s, billing_address = %s 
                WHERE user_id = %s
            """, (card_number, expiry_date, cvv, cardholder_name, billing_address, user_id))
        else:
            # Insert new payment credentials
            cursor.execute("""
                INSERT INTO payment_credentials (user_id, card_number, expiry_date, cvv, cardholder_name, billing_address) 
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (user_id, card_number, expiry_date, cvv, cardholder_name, billing_address))
        
        # Commit the transaction
        connection.commit()
        messagebox.showinfo("Success", "Payment credentials updated successfully.")
    except mysql.connector.Error as err:
        messagebox.showerror("Error", f"Failed to process payment: {err}")

    # After updating payment credentials, you can update wallet balance or perform other actions here if needed
    try:
...
def payment_credentials(user_id):
    payment_window = Toplevel(app)
    payment_window.title("Payment Details")
    payment_window.geometry("400x300")

    card_label = Label(payment_window, text="Card Number:")
    ...
    expiry_label = Label(payment_window, text="Expiry Date (MM/YY):")
    expiry_label.grid(row=1, column=0, padx=10, pady=10)
    expiry_entry = Entry(payment_window)
    expiry_entry.grid(row=1, column=1, padx=10, pady=10)

    cvv_label = Label(payment_window, text="CVV:")
    cvv_label.grid(row=2, column=0, padx=10, pady=10)
    cvv_entry = Entry(payment_window)
    cvv_entry.grid(row=2, column=1, padx=10, pady=10)

    name_label = Label(payment_window, text="Cardholder Name:")
    name_label.grid(row=3, column=0, padx=10, pady=10)
    name_entry = Entry(payment_window)
    name_entry.grid(row=3, column=1, padx=10, pady=10)

    address_label = Label(payment_window, text="Billing Address:")
    address_label.grid(row=4, column=0, padx=10, pady=10)
    address_entry = Entry(payment_window)
    address_entry.grid(row=4, column=1, padx=10, pady=10)

    payment_btn = Button(payment_window, text="Make Payment", command=lambda: [process_payment(user_id, card_entry.get(), expiry_entry.get(), cvv_entry.get(), name_entry.get(), address_entry.get()), payment_window.destroy()])
    payment_btn.grid(row=5, column=0, columnspan=2, pady=10)
...
def process_payment(user_id, card_number, expiry_date, cvv, cardholder_name, billing_address):
    ...
    if not (card_number and expiry_date and cvv and cardholder_name and billing_address):
        messagebox.showerror("Error", "Please fill in all payment details.")
        return

    # Perform additional validation if needed (e.g., check card number format, expiry date format, etc.)
    # You can use regular expressions or other validation methods for this.

    # Assuming payment is successful, deduct payment amount from the user's wallet balance
    payment_amount = 100  # Example payment amount
    current_balance = get_wallet_balance(user_id)
    if current_balance is not None and current_balance >= payment_amount:
    ...
    # Insert payment credentials into the database
    try:
        cursor.execute("INSERT INTO payment_credentials (user_id, card_number, expiry_date, cvv, cardholder_name, billing_address) VALUES (%s, %s, %s, %s, %s, %s)",
                       (user_id, card_number, expiry_date, cvv, cardholder_name, billing_address))
        connection.commit()
        messagebox.showinfo("Success", "Payment successful. Thank you!")
    except Exception as e:
...
def admin_login():
    ...
    # Widgets
    label = Label(admin_login_window, text="Admin Login")
    label.pack()

    username_label = Label(admin_login_window, text="Username:")
    username_label.pack()
    username_entry = Entry(admin_login_window)
    username_entry.pack()

    password_label = Label(admin_login_window, text="Password:")
    password_label.pack()
    password_entry = Entry(admin_login_window, show="*")
    password_entry.pack()

    login_button = Button(admin_login_window, text="Login", command=lambda: admin_authenticate(username_entry.get(), password_entry.get(), admin_login_window))
    login_button.pack()

def admin_authenticate(username, password, window):
    """
    Authenticate admin credentials.
    """
    # Check the admin username and password against stored values
...
def admin_dashboard():
    ...
    """
    admin_dashboard_window = Toplevel()
    ...
    # Create a Treeview widget to display user data in a tabular format
    tree = ttk.Treeview(admin_dashboard_window)
    tree['columns'] = ('user_id', 'full_name', 'pan_number', 'aadhar_number', 'phone_number')
    ...
    tree.column('full_name', anchor='center', width=150)
    tree.heading('pan_number', text='PAN Number')
    tree.column('pan_number', anchor='center', width=100)
    tree.heading('aadhar_number', text='Aadhar Number')
    tree.column('aadhar_number', anchor='center', width=120)
    ...
    for index, user in enumerate(users):
        tree.insert('', 'end', text=index + 1, values=(user[0], user[1], user[2], user[3], user[4]))
...
#     window.attributes('-fullscreen', True)
#     window.bind('<Escape>', lambda e: window.attributes('-fullscreen', False))  # Pressing 'Escape' will exit full-screen mode
# Update page1() function to include admin login button
def page1():
    """
    ...
    admin_login_btn = CTkButton(master=frame, text="Admin Login", command=admin_login)
    admin_login_btn.place(x=30, y=320)
